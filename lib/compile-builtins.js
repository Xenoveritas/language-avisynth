...
Builtins.prototype = {
  ...
  processFile: function(filename) {
    ...
    delete data['meta'];
    var collapse = {};
    if (meta && meta['collapse']) {
      if (util.isArray(meta['collapse'])) {
        // Convert to a set, basically.
        meta['collapse'].forEach(function(path) {
          collapse[path] = true;
        });
      }
    }
    this._recurseTypes("", data, collapse);
  },
  ...
  _recurseTypes: function(path, types, collapse) {
    for (var k in types) {
      var o = types[k], p;
      if (path.length == 0) {
        // Must use the current path name
        p = k;
      } else if (path in collapse) {
      ...
      if (util.isArray(o)) {
        ...
        this._recurseTypes(p, types[k], collapse);
      } else {
