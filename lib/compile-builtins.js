...
Builtins.prototype = {
  ...
  createAutocomplete: function() {
    ...
    for (var k in this.types) {
      var key = k, idx = k.indexOf('.'), suggestions;
      if (idx >= 0) {
        key = k.substring(0, idx);
      }
      if (key in symbols) {
        symbols[key].selector += ', .' + k;
        suggestions = symbols[key].suggestions;
      } else {
        suggestions = [];
        symbols[key] = {
          selector: '.' + k,
      ...
      suggestions.push.apply(suggestions, this.types[k]);
    }
    return { autocomplete: symbols };
  },
  /**
  ...
  writeAutocompleteCSON: function(dest, options) {
    if (!dest)
      dest = process.stdout;
    dest.write(CSON.stringify(this.createAutocomplete()));
  }
}
...
if (module.parent == null) {
  ...
  var args = require('minimist')(process.argv.slice(2), {
    ...
    'boolean': [ 'dump-patterns', 'autocomplete' ]
  });
  ...
  if (args['autocomplete']) {
    processor.writeAutocompleteCSON();
